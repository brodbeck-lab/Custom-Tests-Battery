...
# Import the Stroop Color-Word Task
from stroop_colorword_task import StroopColorWordTask
...
class SelectionMenu(QMainWindow):
    ...
    def test_selected(self, test_name):
        ...
        elif clean_test_name == "Letter Monitoring Task":
            self.launch_letter_monitoring_task(x_pos, y_pos)
    ...
    def launch_stroop_task(self, x_pos, y_pos):
        """Launch Stroop Color-Word Task with enhanced error handling."""
        try:
            print(f"Launching Stroop Color-Word task with:")
            ...
            # Create Stroop Color-Word task with recovery support
            ...
            print("Stroop Colour-Word Task launched successfully with crash recovery")
    ...
    def handle_task_launch_error(self, task_name, error):
        """Handle errors during task launch with emergency save."""
        print(f"Error launching {task_name}: {str(error)}")
        import traceback
        traceback.print_exc()
        ...
        if session_manager:
            try:
                session_manager.emergency_save()
                print(f"Emergency save completed due to {task_name} launch error")
            except Exception as save_error:
                print(f"Emergency save failed: {save_error}")
        
        QMessageBox.critical(self, "Launch Error", 
                           f"Failed to launch {task_name}!\n\n"
                           f"Error: {str(error)}\n\n"
                           f"Please try again or contact support if the problem persists.")
...
def main():
    ...
    # buttons_size: Controls the size of all six test selection buttons (default: 1.0)
    # buttons_elevation: Controls how much the neumorphic buttons stand out (default: 1.0)
